...
export class SessionManager {
  ...
  private async provisionAndStart(
    ...
    try {
      ...
      // If the user has a GitHub App installation, surface the account
      // login as GITHUB_ACCOUNT_LOGIN so the agent's ~/.gitconfig writer
      // can seed git user.name/email + write the credential-helper config.
      // We deliberately do NOT inject the token itself into env — the
      // helper fetches one server-side per git operation via
      // /api/agent/github/token, keeping raw tokens off the workspace
      // filesystem, out of process envs, and out of backups.
      const env: Record<string, string> = {
      ...
      try {
        ...
        if (install) env["GITHUB_ACCOUNT_LOGIN"] = install.accountLogin;
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
