...
### TLS strategy surface (PR #62, 2026-05-06)
...
**Compose layout:** the base `compose/docker-compose.yml` is **TLS-mode-agnostic** — no `--certificatesresolvers.*` flags. Mode-specific config lives in `compose/traefik.override.yml`, generated by the installer + `agenthub reconfigure-tls`. `.env` carries `COMPOSE_FILE=docker-compose.yml:traefik.override.yml` for non-localhost installs. **Don't put TLS flags back into the base compose** — they belong in `packages/installer/src/lib/tls/render-override.ts`.

**Loud-failure gate** (`packages/installer/src/headless.ts` `probeFrontDoor`): after install, the cert is read via `openssl s_client` and the install fails (exit 3) if Traefik is serving its built-in `CN=TRAEFIK DEFAULT CERT`. **Never** silently fall back — if you're touching the install flow, preserve this gate.
...
## Common gotchas
- **Cert flags belong in the override, not the base compose.** Adding `--certificatesresolvers.le.acme.*` back to `compose/docker-compose.yml` re-introduces the silent-fallback bug killed by PR #62. The override file is generated; don't hand-edit it either — use `agenthub reconfigure-tls`.
